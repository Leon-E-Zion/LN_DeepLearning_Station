...
class PPSegLite(nn.Layer):
    ...
    def __init__(self, num_classes, pretrained=None, align_corners=False):
        ...
        self.conv_bn2 = _ConvBNReLU(36, 18, 1, 1, 0)
        ...
        self.depthwise_separable1 = _SeparableConvBNReLU(82, 46, 3, stride=1)
    ...
    def forward(self, x):
        ...
        shortcut_0 = self.conv_bn1(x)  # shortcut
        shortcut_1= F.max_pool2d(x, kernel_size=3, stride=1, padding=1)#一层1/2的shortcut 以便于skip connection
        x = F.max_pool2d(x, kernel_size=3, stride=2, padding=1)  # 1/4
        shortcut_1 = self.conv_bn2(shortcut_1) 
        x = self.block1(x)  # 1/8
        x = self.block2(x)  # 1/16

        # Decoder
        x = self.depthwise_separable0(x)
        shortcut_0_shape = paddle.shape(shortcut_0)[2:]
        x = F.interpolate(
            x,
            shortcut_0_shape,
            mode='bilinear',
            align_corners=self.align_corners)
        x = paddle.concat(x=[shortcut_0, x], axis=1)
        x = self.depthwise_separable1(x)
        shortcut_1_shape = paddle.shape(shortcut_1)[2:]
        x = paddle.concat(x=[shortcut_1, x], axis=1)
